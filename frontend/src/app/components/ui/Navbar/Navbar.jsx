  ...
  useEffect(() => {
    const checkExpirations = async () => {
       try {
         ...
         const [choferes, camiones, semirremolques, cobranzasNac, cobranzasInt, pagosNac, pagosInt] = await Promise.all([
         ...
         // Chequear Choferes
         (choferes || []).forEach(c => checkDate(c.licencia, c.nombre, "Licencia", c._id));
         // Chequear Camiones
         (camiones || []).forEach(c => checkDate(c.vencimiento_seguro, `Camión ${c.patente}`, "Seguro", c.patente));
         // Chequear Semis
         (semirremolques || []).forEach(s => checkDate(s.vencimiento_seguro, `Semi ${s.patente}`, "Seguro", s.patente));
