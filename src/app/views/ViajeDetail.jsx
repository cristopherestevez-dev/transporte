...
import { HiArrowLeft, HiDownload, HiCurrencyDollar } from "react-icons/hi"; // Changed Icon
  ...
  useEffect(() => {
    async function fetchTrip() {
      try {
        ...
        if (type === "nacional") {
            foundTrip = data.viajesNacionales.find(t => t.id.toString() === id.toString());
        } else if (type === "internacional") {
            foundTrip = data.viajesInternacionales.find(t => t.id.toString() === id.toString());
        } else {
             // Fallback: try searching both if type logic fails or url is manual
             foundTrip = data.viajesNacionales.find(t => t.id.toString() === id.toString()) || 
                         data.viajesInternacionales.find(t => t.id.toString() === id.toString());
        }
        
        if (foundTrip) {
            // Resolve relationships (Proveedores, Fleteros, Choferes) if IDs exist
            const provider = data.proveedores?.find(p => p.id === foundTrip.proveedorId);
            const fletero = data.fleteros?.find(f => f.id === foundTrip.fleteroId);
            // Driver logic matches ChoferProfile logic (asignadoId or choferId)
            let driver = null;
            if (foundTrip.tipoAsignacion === 'chofer' && foundTrip.asignadoId) {
                driver = data.choferes?.find(c => c.id === foundTrip.asignadoId);
            } else if (foundTrip.choferId) {
                driver = data.choferes?.find(c => c.id === foundTrip.choferId);
            }
    ...
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
            ...
            <div>
            <h1 className="text-2xl font-bold text-gray-800">Detalle del Viaje #{trip.id}</h1>
